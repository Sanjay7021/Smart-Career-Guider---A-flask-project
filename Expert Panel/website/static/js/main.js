...
const editQuizDiv = document.getElementById("editQuizDiv");
...
function createQuestions() {
  ...
  for (let i = 1; i <= numQuestions; i++) {
    ...
    let questionbr1 = document.createElement("br");
    ...
    questionDiv.appendChild(questionbr);
    questionDiv.appendChild(questionbr1);
    ...
    for (let j = 1; j <= numAnswers; j++) {
      ...
      answerInput.setAttribute("name", "answer" + i);
      answerInput.setAttribute("id", "answer" + i + j);
      let answerText = document.createElement("input");
      answerText.setAttribute("type", "text");
      answerText.setAttribute("name", "answerText" + i + j);
      answerText.setAttribute("id", "answerText" + i + j);
...
function showResults() {
  ...
  let questionElements = Array.from(quizDiv.children).filter(
    (element) => element.tagName.toLowerCase() === "div"
  );

  for (let i = 0; i < questionElements.length; i++) {
    let question = questionElements[i];
    let questionText = question.querySelector("input[type=text]").value;
    let answers = question.querySelectorAll("input[type=radio]");
    let correctAnswer = "";
    for (let j = 0; j < answers.length; j++) {
      let answer = answers[j];
      if (answer.checked) {
        correctAnswer = answer.nextSibling.value;
        break;
      }
    }
    let answerChoices = [];
    let answerTexts = question.querySelectorAll("input[type=text]");
    for (let k = 0; k < answerTexts.length; k++) {
      answerChoices.push(answerTexts[k].value);
    }
    quiz.push({
      title: title,
      cans: cans,
      ...
      choices: answerChoices.slice(1, 5), // Store only the first four choices
  ...
  fetch("/assign_quiz", {
    ...
    .then((quizJson) => console.log(quizJson))
...
function deletequiz(title) {
  fetch("/delete-quiz", {
    ...
    window.location.href = "/manage_quiz";
  });
}

//activate quiz code
function actquiz(title) {
  fetch("/active-quiz", {
    method: "POST",
    body: JSON.stringify({ title: title }),
  }).then((_res) => {
...
// edit material
// function deletequiz(taskid) {
//   fetch("/edit_task", {
//     method: "POST",
//     body: JSON.stringify({ taskid: taskid }),
//   }).then((_res) => {
//     window.location.href = "/assign_task";
//   });
// }
